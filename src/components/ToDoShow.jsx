...
export default function ToDoShow({ singleToDo, deleteTask, editTask, markAsCompleted }) {
    const handleDelete = () => {
        ...
        <ul>
          <motion.li
            ...
            className={`py-5 rounded-lg text-2xl px-5 flex justify-between ${
              singleToDo.completed ? 'bg-green-400 text-white' : 'bg-gray-400 text-white'
            }`}
          >
            <span
              onClick={handleComplete}
              className={`cursor-pointer mr-4 ${
                singleToDo.completed ? 'text-gray-300 line-through' : 'text-gray-100'
              }`}
            >
              {singleToDo.todoName}
            </span>
